...
// CONFIGURACIÓN (Vienen de las variables de entorno de Vercel)
const SHEET_ID = process.env.GOOGLE_SHEET_ID;
const CLIENT_EMAIL = process.env.GOOGLE_CLIENT_EMAIL;
// El private key a veces da problemas con los saltos de línea al pasarlo por variables, esto lo arregla:
const PRIVATE_KEY = process.env.GOOGLE_PRIVATE_KEY.replace(/\\n/g, '\n');
const WHATSAPP_TOKEN = process.env.WHATSAPP_TOKEN;

module.exports = async (req, res) => {
  // 1. VERIFICACIÓN DE META (Solo ocurre cuando configuras el webhook por primera vez)
  if (req.method === 'GET') {
  ...
  if (req.method === 'POST') {
    try {
      ...
      const from = messageObj.from; // Número del remitente
...
async function procesarMensaje(telefono, mensaje) {
  ...
  // EXPRESIONES REGULARES (Para entender qué escribe el humano)
  ...
  if (mensaje.match(regexEntrada)) {
    ...
    if (filaEncontrada) {
      const saldoActual = parseInt(filaEncontrada.B1 || filaEncontrada._rawData[1] || 0); // Ajuste según nombre columna
      ...
      // Si usaste "A1 (Referencia)", cambiaremos la lógica abajo.
      
      // Actualizamos Saldo (Suponiendo cabeceras "Referencia" y "Cantidad")
      // Si usaste A1/B1 en la fila 1, la librería las toma como headers.
      ...
      respuesta = `❌ Error: La referencia ${ref} no existe en la hoja Inventario. Créala primero.`;
    }
    ...
    if (filaEncontrada) {
      ...
      if (saldoActual < cant) {
        respuesta = `⚠️ *ALERTA*: No hay suficiente inventario.\nTienes: ${saldoActual}\nIntentas sacar: ${cant}`;
      } else {
        const nuevoSaldo = saldoActual - cant;
        filaEncontrada.Cantidad = nuevoSaldo;
        await filaEncontrada.save();
